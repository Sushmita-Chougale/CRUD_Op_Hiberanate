...
<%
	...
	try {
		...
		ResultSet rs = stmt.executeQuery("select customer_details.first_name, customer_details.middle_name, customer_details.last_name ,p.product_name, p.village, p.company, p.weight, p.quantity, p.salePriceWithoutTax, p.total_per_product, p.total_all_product, p.transportation_expense, p.hamali_expense, p.gross_total, insert_date,p.tax_percentage,p.hsn,p.iGst from pesticide_billing p left join customer_details on p.fk_customer_id = customer_details.pk_customer_id where p.bill_no =" + billno);
